...
### STRIPE new ###
# Stripe yapida bir [VG] olusturulmasi desteklenir.
...
echo
;;
*swap*)echo "[$disk] diski ('swap alani') iceriyor.. [PV] diski olarak kullanilamaz !! " ; errormsg
;;
...
pvscheck() {
## ilgili diskin PV diski oldugundan emin olunmasi gerekli !!
ispvs=0
...
### vgx ilk kontrol !!! ###
## VG Name contains invalid character, valid set includes: [a-zA-Z0-9.-_+].
...
"ext")
if [ -z "$pvc" ] ; then
## ext parametresinde pvcreate yapilmamasi daha mantikli olur ##
#pvfscheck
printit 30
...
grow_devdiskp=$(echo $grow_devdisk|sed 's/[0-9]\+$//')
[[ ! -z "$freerawgib" ]] && \
echo "[ growpart $grow_devdiskp $diskprt ] komutu ile bu ( $freerawgib Gib ) $FREE_RAW RAW alani [$disk] partitiona aktarmayi deneyebilirsiniz .. " || \
echo "[ growpart $grow_devdiskp $diskprt ] komutu ile bu $FREE_RAW RAW alani [$disk] partitiona aktarmayi deneyebilirsiniz .. "
...
printit 30
grow_devdiskp=$(echo $grow_devdisk|sed 's/[0-9]\+$//')
echo "'growpart $devdiskp $diskprt' komutu ile bu Free alani [$disk] partitiona aktarip bu alani da kullanabilirsiniz .. "
...
###############
## nvme ##
nvme_rescan=$(echo "$diskdx"|sed 's/\(nvme[0-9]*\).*/\1/')
[[ "$diskother" = "ok" ]] && echo 1 > /sys/class/nvme/$nvme_rescan/device/rescan || echo 1 > /sys/block/$diskdx/device/rescan
partsizechk "$devdisk"
...
## sadece LV/VG isimleri
### lvs --noheadings -o lv_name ###
### vgs --noheadings -o lv_name ###
...
if [ -z "$freevggib" ] ; then
freevggib=$(vgs --noheadings -o vg_free "$vgx" 2>/dev/null |awk '$NF=="0"{exit}$NF~/</{sub("<","",$NF)}$NF~/k|K/{x=$NF/1024/1024}$NF~/m|M/{x=$NF/1024}$NF~/t|T/{x=$NF*1024}$NF~/g|G/{x=$NF;}{;printf "%0.2f",x}')
freevggib2=$(vgs --noheadings -o vg_free "$vgx" 2>/dev/null |awk '$NF=="0"{exit}$NF~/</{sub("<","",$NF)}$NF~/k|K/{x=$NF/1024/1024}$NF~/m|M/{x=$NF/1024}$NF~/t|T/{x=$NF*1024}$NF~/g|G/{x=$NF;}{;printf "%0.9f",x}')
else
...
echo
grow_devdiskp=$(echo $grow_devdisk|sed 's/[0-9]\+$//')
echo "[growpart $grow_devdiskp $diskprt] komutunu calistirmaniz gerekiyor olabilir ... "
sleep 2
echo
...
freelv=0
;;
*)echo "Sistem destegi bulunmamaktadir.. $(uname -r) "
freelv=0
;;
esac
printit 50
...
lv_naming_rules() {
echo 'LVM2 allows volume group names and logical volume names to contain characters A to Z, a to z, 0 to 9, underscore (_), hyphen (-), dot (.), and plus (+). 
The names cannot begin with a hyphen.

Reserved names and character strings that are used internally by LVM cannot be used as volume group names or logical volume names. 
A volume group cannot be called anything that exists in /dev/ at the time of creation. 
It cannot be named '.' (a single dot) or '..' (double dot).
...
fstab_check() {
isfstabinready=0
checkl=$(grep -w /$(echo "$mp"|sed 's/[_-]/\./g') /etc/fstab )
if [ ! -z "$checkl" ] ; then
isfstabinready=1
echo
...
echo "MP (Mount_Point) dizin yetkilendirmesi kullaniciya birakilmistir .. "
...
printit 40
sleep 3
echo -e "-> [$vgdiskchk] [VG] icin yapilacak islemi kesmek icin 'Ctrl-C' tuslarini kullanabilirsiniz !! \n"
sleep 2
echo -e "-> [$vgdiskchk] [VG] icin yapilacak islemi kesmek icin 'Ctrl-C' tuslarini kullanabilirsiniz !!! \n"
sleep 1
echo
...
lvchecklvs "$vgx" "$lvx"
if [ ! -z "$islvsx" ] ; then
[[ "$lastp" == "new" ]] && echo "-> [$isvgx] [VG] uzerinde [$lvx] isminde zaten bir [LV] device bulunuyor [FAIL] "
...
printit 30
sleep 3
[ ! -z "$size" ] && echo "->> [VG] olusturulurken [size: $sizec] bilgisi kullaniliyor [INFO] " && echo && sleep 2
...
if [[ "${@: -1}" != "ext" ]] && [[ "${@: -1}" != "new" ]] ; then
echo
echo "-> Son parametre olarak [ext] yada [new] degerleri girilebilir !! "
echo
sleep 1
fi
...
# Girilen tüm diskleri bir diziye (array) aktar
girilen_diskler=("$@")

# Gecici olarak boyutları toplayacağımız diziler
declare -A disk_bayt_boyutlari
declare -A disk_gb_boyutlari
benzersiz_boyut_listesi=""

#echo "Diskler kontrol ediliyor ..."
printit 30
...
for disk in "${girilen_diskler[@]}"; do
    ...
    gb_size=$(lsblk -do SIZE "/dev/$disk" | tail -n 1 | tr -d '[:space:]')

    # Bilgileri hafızaya kaydet
...
if [ "$unique_sizes_count" -gt 1 ]; then
    ...
    printit 30
    for disk in "${girilen_diskler[@]}"; do
        echo "$disk boyutu: ${disk_gb_boyutlari[$disk]}"
    done
    printit 30
    errormsg
else
    ilk_disk="${girilen_diskler[0]}"
    echo "BASARILI: Girilen tum disklerin (${girilen_diskler[*]}) boyutlari tamamen ayni (${disk_gb_boyutlari[$ilk_disk]})."
    echo
    printit 40
    sleep 1
fi
...
echo
echo "Guvenlik icin diskler son 1 kez daha kontrol ediliyor ... "
sleep 2
...
#### Disk size kontrolu ####
echo "Disk size kontrolleri yapiliyor ... "
sleep 1
stripe_disk_size_check ${stripedisks[@]}
####
}
...
echo "[striped] [VG] extend islemleri icin gecerli bir [VG] bilgisi girilmelidir !!! "
sleep 1
errormsg
fi
else
...
## stripe lv check ##
[[ -z "$lvx" ]] && echo "-> [LV] bilgisi otomatik olusturulacak [$lastp] " && lvnamecreate "$vgx" "$lastp"
[[ -z "$lvx" ]] && echo "Stripe [LV] bilgisi olusturulamadi !! " && errormsg


lv_tmp_x=$(lvs -o+lv_layout|awk -va="$vgx" -vb="$lvx" '$2==a&&$1==b')
if [[ -z "$lv_tmp_x" ]] ; then
if [[ "$lastp" = "ext" ]] ; then
echo "[$vgx] [VG] ile eslesen [$lvx] isminde bir [LV] device bulunamadi !! "
errormsg
fi
fi
...
stripe_vg_search $@
stripe_param=1
echo "[striped] parametresi icin [VG-Striped] islemleri yapilacak ... "
fi
}
...
echo "Yeni multipath devicelarin sistemde goruntulenmesi icin [sg3_utils] paketi icinde yer alan [rescan-scsi-bus.sh] scriptini calistirabilirsiniz .. "
sleep 1
...
if [ $pvslc -eq 0 ] ; then
[[ "$lastp" == "new" ]] && echo "-> [$disk] diski bir [PV] disk olarak gorunmuyor [INFO] "
[[ "$lastp" == "ext" ]] && echo "-> [$disk] diskinin bir [PV] disk oldugundan emin olun [FAIL] "
sleep 1
elif [ $pvslc -eq 1 ] ; then
pvlayout=$(pvs "${disk_layout_dev}" -o+lv_layout,stripes 2>/dev/null|awk 'NR>1{print $(NF-1)}')
...
printit 40
pvlayout=$(pvs "${disk_layout_dev}" -o+lv_layout,stripes 2>/dev/null|awk 'NR>1{a[$(NF-1)]++}END{for(i in a)print i}')
...
sleep 1
pvlayout=$(vgs "$vgx_layout" -o+lv_layout | awk 'NR>1{print $NF;exit}')
fi
printit 50
...
###
vg_empty=$(pvs "${disk_layout_dev}" -o+lv_layout,stripes 2>/dev/null|awk 'NR>1{if($2=="lvm2")print "OK"}')
if [[ ! -z "$vg_empty" ]] ; then
if [[ "$lastp" == "new" ]] ; then
...
if [[ -z "$vgsfchk" ]] ; then
if [[ "$lastp" == "ext" ]] ; then
[[ "$pvlayout" == "linear" ]] && echo "->> [$vgx] isminde Linear yapida aktif bir [VG] bulunamadi [FAIL] " && sleep 1
...
vgs "$vgx" &>/dev/null
if [ $? -ne 0 ] ; then
vgs 2>/dev/null|grep -q "$vgx"
if [ $? -eq 0 ] ; then
vgxx=$(vgs 2>/dev/null|grep "$vgx"|awk '{print $1}')
echo "->> DIKKAT : Benzer isimde [VG] bilgisi tespit edildi ->>" [$vgxx]
sleep 1
echo "->> DIKKAT : Benzer isimde [VG] bilgisi tespit edildi ->>" [$vgxx]
sleep 1
echo
...
if [[ "$lastp" = "new" ]] ; then
echo "-> [new] parametresi kullanilarak [$vgx] isimli yeni bir [VG] create edilecektir .. "
sleep 1
echo
elif [[ "$lastp" = "ext" ]] ; then
echo "-> [$vgx] isminde gecerli bir [VG] bilgisi bulunamadigi icin extend islemleri yapilamiyor !!! "
echo
errormsg
fi
fi
