...
( multipath -l $mpath ; echo "" ) >> mpathsds
done < mpaths 
...
awk 'NR==FNR{a[$1];next}
{if($4 in a){c=0;for(i=1;i<=9;i++){if(!($4 i in a))c++};if(c==9){$4=$4"[no kernel partition]";b[x++]=$0}else b[x++]=$0}else b[x++]=$0}
END{for(j=0;j<x;j++)print b[j]}
' diskparts full2 > full3

ls -ltr /dev/mapper/mpath* |sed 's/[^ ]* [^ ]* [^ ]* [^ ]* \([^ ]*,\) *\([^ ]*\).*\/\(.*\)$/\1\2 \3/' > mpaths
ls -ltr /dev/dm*|sed 's/[^ ]* [^ ]* [^ ]* [^ ]* \([^ ]*,\) *\([^ ]*\).*\/\(.*\)$/\1\2 \3/' > dms
awk 'NR==FNR{a[$1]=$2;next}{if($1 in a)print $1,$2,a[$1]}' mpaths dms > full4

awk 'NR==FNR{a[$NF]=$3 FS $2 FS $1;next}
{if($1 in a){printf "%30s\n%15s%15s",$0,"["a[$1]"]","[PARTITIONS]";for(i=0;i<=9;i++){if(a[$1"p"i])printf "%20s"," \\__ ["a[$1"p"i]"]"}printf "%s","\n"} else print}' full4 full3 > full5
...
awk 'NR==FNR{sub("rport-","",$NF);sub("-",":",$NF);;fullrec[$NF]=$1 FS $2 FS $3;;if($0~"Fibre")hbaports[$1]=$0}
{
if($1~/mpath/||/size/||/prio/)print;
else{
if(!NF){
for(hba in writehba){
if(writehba[hba]){
...
if($2 in fullrec)print fullrec[$2] FS $0}}
...
check_mpaths() {
multipath -l|awk '$1~"mpath"{mps[$1]++}END{for(i in mps)print i}' > multipaths
...
sddev() {
awk -vdev="$1" '$1~"mpath"{b=$1;while(getline&&NF)a[b]=a[b] RS $0}
END{for(i in a){if((a[i]~dev FS)||(a[i]~dev"\\["))print RS i,dev RS"============",a[i];else c++}
